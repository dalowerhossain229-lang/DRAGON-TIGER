...
// 🛫 ২. ড্রাগন টাইগার কোর কার্ড ডিলিং রাউট (১.৯৫ ওডস ও কঠোর ২০০০০ লিমিট সিকিউরিটি ফিল্টার লক ভাই ভাই!)
app.post('/api/dragon-deal', async (req, res) => {
    ...
    try {
        ...
        let adminTriggeredPrize = (balResponse.data && balResponse.data.dragon_target) ? balResponse.data.dragon_target : null;

        let dragonCard, tigerCard, finalResultSide, finalStatus, winMultiplier;
        let isLoopActive = true;
        let loopSafety = 0;

        // 🎰 [🎰 ৯৫% ওরিজিনাল ক্যাসিনো RTP ড্রাগন টাইগার গাণিতিক লুপ ভাই ভাই]
        while (isLoopActive && loopSafety < 200) {
            loopSafety++;
            
            // ড্রাগন এবং টাইগারের জন্য ১টি করে র্যান্ডম তাস জেনারেটর (মান ১ থেকে ১৩)
            let dVal = Math.floor(Math.random() * 13) + 1;
            let tVal = Math.floor(Math.random() * 13) + 1;

            dragonCard = { value: dVal, suit: cardSuitsPool[Math.floor(Math.random() * 4)] };
            tigerCard = { value: tVal, suit: cardSuitsPool[Math.floor(Math.random() * 4)] };

            if (dVal > tVal) finalResultSide = "DRAGON";
            else if (tVal > dVal) finalResultSide = "TIGER";
            else finalResultSide = "TIE";

            if (userPrediction === finalResultSide) {
                finalStatus = "win";
                // 🚀 [১.৯৫ ওডস প্রফিট বুস্টার ম্যাথ]: ২.০০ গুণের পরিবর্তে সরাসরি ১.৯৫ গুণ প্রফিট লক (টাই হলে স্পেশাল ৮ গুণ)!
        ...
        if (finalStatus === "win") {
            // 🚀 [দশমিক ফিক্স]: ১.৯৫ ওডসের নিখুঁত দশমিক উইন পেমেন্ট সিঙ্ক ভাই ভাই!
...
io.on('connection', (socket) => { console.log("Player connected to Royal Dragon Tiger Engine!"); });

// ড্রাগন টাইগার গেম নিজস্ব কাস্টম ৪০০০ পোর্টে কড়া নিয়নে অন ফায়ার ভাই ভাই!
const PORT = process.env.PORT || 19000; 
server.listen(PORT, () => { console.log(`🎡 Royal Dragon Tiger Engine Running on port ${PORT}`); });
